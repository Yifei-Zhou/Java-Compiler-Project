...
ANTLR program has a `.g4` file to define the structure of the programming language. In this project, the structure of `MicroC` is defined in `java/MicroC.g4`, the details of tokens and grammars definitions can be found in the [ANTLR website](https://www.antlr.org/).
...
The following codes in `java/compiler/Compiler.java` shows how to use the Scanner to turn an input stream into a stream of tokens; then how to pass this stream of tokens to the Parser to build a parse tree.
...
The following code use `java/compiler/Compiler.java` to compile the parser and lexer together to create a program. This program is used to parse any program and check if it matches the grammar.
```
javac -cp $CLASSPATH -d classes java/*.java build/*.java
```
